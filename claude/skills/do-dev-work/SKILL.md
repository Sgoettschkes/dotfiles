...
- **The new instance** (started by `ao-open-worktree`): step 5 — implementation via `/phx:full` plus walk-through verification. The hand-off instruction tells it so.
...
- All context: the ticket, any prior/related **PR** and the **Epic** (URLs), plus a one-liner on how they relate — read these out of the ticket / the user's request and inline them.
...
## 5. Implementation & walk-through — runs in the new instance

For reference (not here):
1. `/phx:full` runs plan → implement → review → commit against the ticket, using the PR/Epic context.
2. After verifying: user-visible/visual changes → invoke **walk-through-change** to boot the dev server, seed the DB, and open the changed page logged-in; nothing visual → skip and say why.

## Done

Report back: the ticket (title + URL), its claimed state, the branch/worktree path and ports, and that the new session is running `/phx:full`. This instance's job ends there.
